...
class DSPApplication(QMainWindow):
    ...
    def generate_signal_by_type(self, signal_type, amplitude, start_time, duration, sampling_rate=None):
        ...
        if signal_type == UNIFORM_NOISE:
            ...
            return SignalGenerator.generate_square_wave(amplitude, period, start_time, duration, duty_cycle,
                                                        sampling_rate)
        elif signal_type == SYMMETRICAL_SQUARE_WAVE:
            period = self.periodic_parameter_inputs[PERIOD].value()
            duty_cycle = self.duty_cycle_parameter_inputs[DUTY_CYCLE].value()
            return SignalGenerator.generate_symmetric_square_wave(amplitude, period, start_time, duration, duty_cycle,
                                                                  sampling_rate)
        elif signal_type == TRIANGULAR_WAVE:
            period = self.periodic_parameter_inputs[PERIOD].value()
            duty_cycle = self.duty_cycle_parameter_inputs[DUTY_CYCLE].value()
            return SignalGenerator.generate_triangular_wave(amplitude, period, start_time, duration, duty_cycle,
                                                            sampling_rate)
        elif signal_type == UNIT_STEP_FUNCTION:
            ...
            return SignalGenerator.generate_unit_impulse(amplitude, first_sample_index, peak_sample_index,
                                                         number_of_samples, sampling_rate)
        elif signal_type == UNIT_NOISE:
